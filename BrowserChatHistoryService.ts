...
export default class BrowserChatHistoryService extends ChatHistoryService {
	/** @type {string} */
	name: string = "BrowserChatHistoryService";
	/** @type {string} */
	storageKeyPrefix!: string;
	/** @type {string} */
	sessionsListKey!: string;
	...
	constructor(storageKeyPrefix?: string) {
		...
		console.log(
			`BrowserChatHistoryService initialized with prefix: '${this.storageKeyPrefix}' and session list key: '${this.sessionsListKey}'`,
		);
	}
	 ...
	 */
	_getSessionsList(): any[] {
		try {
			const storedSessions = localStorage.getItem(this.sessionsListKey);
			return storedSessions ? JSON.parse(storedSessions) : [];
		} catch (error) {
			console.error(
				`Error reading sessions list from localStorage (key: ${this.sessionsListKey}):`,
				error,
			);
			return [];
		}
	 ...
	 * @returns void
	 */
	_saveSessionsList(sessions: any[]): void {
		try {
	...
	async createSession(name?: string): Promise<any> {
		...
		const sessionName =
			name && name.trim()
				? name.trim()
				: `Chat @ ${new Date(now).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}`;
	 ...
	 */
	async deleteSession(sessionId: string | number): Promise<boolean> {
	 ...
	 */
	_getSessionKey(sessionId: string | number): string {
		if (!sessionId) {
			console.error(
				"Session ID is required for message operations but was not provided.",
			);
	 ...
	 */
	_getMessages(sessionId: string | number): any[] {
		try {
			...
			console.error(
				`Error reading messages for session ${sessionId} from localStorage:`,
				error,
			);
			return [];
		}
	 ...
	 * @returns void
	 */
	_saveMessages(sessionId: string | number, messages: any[]): void {
		try {
			...
			console.error(
				`Error saving messages for session ${sessionId} to localStorage:`,
				error,
			);
		}
	}
	 ...
	 * @private
	 * @param sessionId - The session identifier
	 * @param lastMessageText - Text from the last message for preview
	 * @returns void
	 */
	_updateSessionMetadata(
		...
		if (sessionIndex > -1) {
			...
			sessions[sessionIndex].previewText =
	 ...
	 */
	async addMessage(sessionId: string | number, message: any): Promise<any> {
		if (!sessionId)
			return Promise.reject(
				new Error("Session ID is required to add a message."),
			);
	 ...
	 */
	async clearSessionHistory(sessionId: string | number): Promise<void> {
		if (!sessionId)
			return Promise.reject(
				new Error("Session ID is required to clear history."),
			);
	 ...
	 *
	 * @param sessionId - The session identifier
	 * @returns Array of messages (flat list)
	 * @throws {Error} When sessionId is missing
	 */
	async getThreadTree(sessionId: string | number): Promise<any[]> {
		if (!sessionId)
			return Promise.reject(
				new Error("Session ID is required for getThreadTree."),
			);
		console.warn(
			"BrowserChatHistoryService: getThreadTree is not fully implemented. Returning flat list.",
		);
	...
	async getChatHistoryByMessageId(messageId: string | number): Promise<any[]> {
		...
		console.warn(
			"BrowserChatHistoryService: getChatHistoryByMessageId requires sessionId but base class doesn't provide it. Returning empty array.",
		);
		return Promise.resolve([]);
	}
	 ...
	 * @param sessionId - The session identifier to search within
	...
	async searchMessages(
		...
		if (!sessionId)
			return Promise.reject(
				new Error("Session ID is required for searchMessages."),
			);
